...
> "我做了一个 AI Code Review 工具，核心亮点是手写了 SSE 流式渲染而不依赖 SDK，用 `ReadableStream` + `TextDecoder` 逐块处理响应，实现打字机效果。同时用 `AbortController` 处理取消重复请求，集成了 Monaco Editor（VS Code 同款），用结构化 Prompt 让 AI 输出标准格式的 Review 报告。"

...
> **类型缩窄是什么？** 类型缩窄是指：TypeScript 根据运行时判断（如 `typeof`、`in`、`instanceof`、判空）把一个“宽类型”收窄成更具体的类型。
>
> **有何作用？**
>
...
> **补充说明**：`interface` 和 `type` 不是“二选一对立关系”，而是各有侧重。常见实践是：
>
...
> **补充说明**：泛型的核心价值是“在复用逻辑的同时保留类型信息”。你可以把它理解为“类型层面的参数化函数”，避免写一堆重复类型。
...
> **补充说明**：这里的 `extends` 读作“满足约束”，不是“继承类”的那层语义。它的作用是给泛型参数加边界：既保留泛型灵活性，又保证你能安全访问某些属性。
...
> **`infer`（你提到的 inter 大概率是这个）是做什么的？** `infer` 只能出现在条件类型的 `extends` 子句里，用来声明一个“待推断的类型变量”，然后从已有类型结构中把这部分类型“提取出来”。
>
> **有何作用？**
>
> - 自动提取函数返回值、参数类型；
...
type CamelToKebab<S extends string> = S extends `${infer L}${infer R}`

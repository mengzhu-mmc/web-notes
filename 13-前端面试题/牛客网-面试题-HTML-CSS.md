...
**常见语义化标签：** `<header>` 页头、`<nav>` 导航、`<main>` 主内容、`<article>` 文章、`<section>` 区块、`<aside>` 侧边栏、`<footer>` 页脚、`<figure>` 图文组合、`<time>` 时间
...
`<script src>` 会阻塞 HTML 解析（所以推荐放 body 底部或加 `async/defer`）。 `<link href>` 加载 CSS 不阻塞 HTML 解析，但会阻塞渲染（CSSOM 构建完才渲染）。

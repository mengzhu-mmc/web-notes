...
<span class="hljs-keyword">for</span>(<span class="hljs-keyword">let</span> i = <span class="hljs-number">0</span>; i &lt; arr.length; i++) { <span class="hljs-keyword">if</span>(<span class="hljs-built_in">Array</span>.isArray(arr[i])) { result = result.concat(flatten(arr[i])); } <span class="hljs-keyword">else</span> { result.push(arr[i]); } } <span class="hljs-keyword">return</span> result; } flatten(a); <span class="hljs-comment">// &nbsp;[1, 2, 3, 4，5]</span> </code></pre>

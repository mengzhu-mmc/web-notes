...
input是要转换的值，PreferredType是可选参数，可以是Number或String类型。它只是一个转换标志，转化后的结果并不一定是这个参数所指的类型，但是转换结果一定是一个原始值（或者报错）。
```
...
```
1.该对象为 Data 类型，则 PreferredType 被设置为 String
2.否则，PreferredType 被设置为 Number
```

##### 1.3 valueOf() 方法和 toString() 方法解析

先看看对象的 valueOf 函数，其转换结果是什么？对于 js 的常见内置对象：Date，Array，Math，Number，Boolean，String，Array，RegExp，Function

1.Number、Boolean、String这三种构造函数生成的基础值的对象形式，通过valueOf 转换后会变成相应的原始值，如：
...
let bool = new Boolean('sdf');
// Boolean {true}
bool.valueOf(); // =>true
```

2. Date 这种特殊的对象，其原型 Date.prototype上内置的valueOf() 函数将日期转换为时间戳的形式

```
let a = new Date();
a.valueOf(); // => 1629793358246
```
...
1. Number、Boolean、String、Array、Date、RegExp、Function这几种构造函数生成的对象，通过 toString 方法后会变成相应的字符串的形式，因为这些构造函数上封装了自己的 toString() 方法。

```
let num = new Number('12a')
num.toString() //=> NaN

let str = new String('12a')
str.toString() //=> '12a'

let bool = new Boolean('ab')
bool.toString() //=> 'true'

let d = new Date()
d.toString() //=> "Tue Aug 24 2021 16:51:40 GMT+0800 (中国标准时间)"
...
除了这些对象及其实例化对象之外，其他对象返回的都是该对象的类型，都是继承的Object.prototype.toString()方法
...
因为valueOf函数会将Number、String、Boolean基础类型的对象类型值转换成 基础类型，Date类型转换为毫秒数，其它的返回对象本身，而toString方法会将所有对象转换为字符串。显然对于大部分对象转换，valueOf转换更合理些，因为并没有规定转换类型，应该尽可能保持原有值，而不应该像toString方法一样，一股脑将其转换为字符串。
...
| Undefined     | false                                                        |
| Null          | false                                                        |
| Boolean       | return argumnet                                              |
| Number        | 仅当 argument 参数是 +0、-0 、NaN-时，return false； 否则freturn true |
| String        | 仅当 argument 参数是 空字符串时， return false;  否则 return true |
| Symbol        | true                                                         |
| Object        | true                                                         |
...
| Number        | 无需转换                                                     |
| String        | 将字符串中的内容转化为数字（比如：“23” => 23），<br>如果转化失败则返回 NaN（比如 “23a” => NaN） |
| Symbol        | 抛出 TypeError 异常                                          |
| Object        | 先 ToPrimitive(argument， Number) 转换得到原始值，<br>再进行 ToNumber 转换为数字 |
...
| Number        | 字符串 转 数字，如：1.2 => '1.2'                             |
| 字符串        | 无需转换                                                     |
| 对象          | 先进行 ToPrimitive(obj，string) 转换得到初始值，<br>再进行 ToString转换为字符串 |

...
> **为何需要 maxLen 而不是 dp[n-1]？** `dp[i]` 只描述"以 i 结尾"的最优解，最后一个元素不一定是最长子序列的结尾（例如 `[1,3,6,7,9,4,10,5,6]`，dp[6]=6 但 dp[8]=5）。需要遍历所有 dp[i] 取全局最大值。

> **关于 `dp[i] = Math.max(dp[i], dp[j]+1)` 的理解**： `dp[i]` 初始为 1（已知保底值，非"未知"），每遍历一个 j 就更新一次临时最大值，类比"每次看到更长的绳子就换手里那根"，遍历完所有 j 后得到最终最大值。

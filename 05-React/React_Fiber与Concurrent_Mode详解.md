...
#### 1.5.1 底层原理：React 17 与 18 到底差在哪

自动批处理的本质，是「多次 `setState` 要不要攒成一次渲染」的**判断依据变了**：

- **React 17——靠「合成事件包裹」判断**：React 在自己的合成事件系统入口设了一个开关（内部的 `isBatchingUpdates`）。进入 React 事件回调（如 `onClick`）时置为 `true`，此时 `setState` 只入队、不立即渲染，回调结束再统一 flush 一次。**一旦代码跑进 `setTimeout` / `Promise.then` / 原生 `addEventListener`，就脱离了这个开关的管辖**，开关是关着的，于是每个 setState 各渲染一次。
- **React 18——靠「优先级 Lane + 调度微任务」判断**：用 `createRoot` 后，每次 `setState` 只给对应 Fiber 标记一条 **Lane（优先级车道）** 并 `ensureRootIsScheduled`，真正的渲染被调度成一个**微任务**延后执行；同一次执行流里的多次 setState 落到同一批 Lane 上，等当前调用栈跑完、微任务触发时**一次性 flush**。判断依据从「代码在哪运行」变成了「渲染在何时发生」，所以无论写在 setTimeout、Promise 还是原生事件里，都能合并。

> **去黑话版**：React 17 看你「在什么地方点菜」——只有坐在餐桌前（事件回调里）说的，服务员才攒着一起下单；打电话追加的（异步里）喊一句就送一次厨房。React 18 改成「不管你在哪点，都先记本子上，等你这一口气说完再统一下单」——所以哪儿点都能攒。`flushSync` 则是「这道菜我急着吃，先单独给我做」，代价是没法和别的攒一起。

#### 1.5.2 关键澄清：批处理的边界是「一个任务」，不是「所有同步代码」

上面说「等当前这段代码跑完再渲染」，这里的「当前这段代码」**不是指程序里所有同步代码，而是指当前这一次连续、不中断的执行**——用 Event Loop 的话说，就是**当前这一个任务（task）**：一个 `onClick` 回调是一个任务、一个 `setTimeout` 回调是一个任务、一个 `Promise.then` 回调是一个任务。**批处理的边界 = 一个任务的边界。**

所以「异步能不能跳过批处理」的答案是：**不能，异步照样被批处理**，区别只在于多个 setState 是否落在同一个任务里：

```js
// 情况 A：三个 setState 在同一个异步回调（同一个任务）里 → 合并成 1 次
setTimeout(() => {
  setA(1);
  setB(2);
  setC(3); // React 18：等这个回调跑完 → 一次性渲染（React 17 会渲染 3 次）
}, 0);

// 情况 B：三个 setState 分散在三个不同的异步回调（三个任务）里 → 各渲染 1 次
setTimeout(() => setA(1), 0); // 任务①
setTimeout(() => setB(2), 0); // 任务②
setTimeout(() => setC(3), 0); // 任务③
// 三个回调是三个独立任务，各自跑完各自渲染 → 3 次
```

结论：

- 批处理**永远不会被自动跳过**（除非主动用 `flushSync`）。
- 它只在**同一个任务内部**把多次 setState 攒成一次。
- 情况 B 的「分开渲染」不是「异步逃过了批处理」，而是它们本就分属不同任务、天然是不同轮次——**这是任务的天然边界，不是漏批**。
- React 17 与 18 的差别只体现在**情况 A**：17 因为不在事件回调里就一句一渲染，18 无论在哪个任务里都会把该任务内的多次更新合并。

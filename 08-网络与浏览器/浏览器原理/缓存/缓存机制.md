...
1. 节省带宽，避免浪费不必要的网络资源
1. 减轻服务器压力
1. 提高浏览器网页的加载速度，提高用户体验

## 2. 缓存分类

1. 强缓存
1. 不会向服务器发送请求，直接从本地缓存中获取数据
1. 请求资源的的状态码为: 200 ok(from memory cache)
1. 协商缓存
1. 向服务器发送请求，服务器会根据请求头的资源判断是否命中协商缓存
1. 如果命中，则返回304状态码通知浏览器从缓存中读取资源
1. 强缓存 & 协商缓存的共同点
1. 都是从浏览器端读取资源
1. 强缓存 VS 协商缓存的不同点
   1. 强缓存不发请求给服务器
...
1. cache-control常用的值（做一个简单了解即可）：
1. no-cache: 不使用本地缓存，需要使用协商缓存。先与服务器确认返回的响应是否被更改，如果之前的响应中存在Etag，那么请求的额时候会与服务器端进行验证，如果资源为被更改则使用缓存。
1. no-store: 直接禁止游览器缓存数据，每次用户请求该资源，都会向服务器发送一个请求，每次都会下载完整的资源。
1. public：可以被所有的用户缓存，包括终端用户和CDN等中间代理服务器。
1. private：只能被终端用户的浏览器缓存，不允许CDN等中继缓存服务器对其缓存。
1. <font color=red>注意：当cache-control与Expires共存的时候cache-control的优先级高</font>
...
强缓存设置了较长的 `max-age` 后，如何让浏览器获取最新资源？

**核心策略：内容变化 → 文件名变化 → 浏览器认为是新资源**

```js
// vite.config.js 配置哈希文件名
export default {
  build: {
    rollupOptions: {
      output: {
        assetFileNames: "[name].[contenthash].[ext]", // 如 main.5f8d72.js
...
- 内容不变 → hash 不变 → 命中强缓存（200 from cache）✅
- 内容变化 → hash 变化 → 浏览器认为是新文件，重新请求 ✅
- HTML 文件本身：设 `Cache-Control: no-cache`，走协商缓存，保证入口文件始终最新

---

## 6. ETag vs Last-Modified 优先级

**ETag 优先级高于 Last-Modified**

| 特性   | Last-Modified                 | ETag                 |
| ------ | ----------------------------- | -------------------- |
| 精度   | 秒级（无法检测1秒内多次修改） | 文件内容哈希，精确   |
| 误判   | 内容未变但修改时间变 → 误重载 | 内容不变则 ETag 不变 |
| 请求头 | `If-Modified-Since`           | `If-None-Match`      |
| 返回   | 304 Not Modified              | 304 Not Modified     |

**浏览器请求流程**：

1. 发送 `If-None-Match: <ETag值>` + `If-Modified-Since: <时间>`
2. 服务端**先验证 ETag**，匹配则返回 304
3. ETag 不匹配，才继续看 Last-Modified
4. 都不匹配，返回 200 + 新资源
...
| 资源类型 | 缓存策略 | 原因 |
| --- | --- | --- |
| JS/CSS/图片（带hash） | `Cache-Control: max-age=31536000, immutable` | 内容变则文件名变，可永久缓存 |
| 入口文件 index.html | `Cache-Control: no-cache` | 需要每次向服务端确认是否最新 |
| API 接口 | `Cache-Control: no-store` 或不设置 | 数据动态变化，不应缓存 |
| 字体/图标（带hash） | `Cache-Control: max-age=31536000` | 同静态资源，内容固定 |

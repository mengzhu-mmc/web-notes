...
> [!tip] 面试一句话 IndexedDB 是浏览器内置的 NoSQL 数据库，支持大容量、结构化存储、索引查询和事务，适合离线应用和需要本地持久化的复杂数据场景。
  ...
  open() {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        const db = event.target.result;
        this.stores.forEach(
          ({ name, keyPath, autoIncrement, indexes = [] }) => {
...
> [!tip] 面试回答要点 IndexedDB 是异步的事务型 NoSQL 数据库，适合存大量结构化数据。核心是「数据库→对象仓库→索引→事务」四层结构。原生 API 回调较多，实际项目通常封装成 Promise 或使用 `idb` 库（Workbox 也内置了封装）。

...
上面的流程图是简化版。把它拆细，一条完整的渲染管线分成 **主线程负责的 1–6 步**（回答“画什么、怎么画、归哪层”）和 **合成线程 / GPU 负责的 7–8 步**（回答“真正画出来、拼上屏”）：
...
1. **DOM / CSSOM 并行构建**。三条阻塞规则：CSS 是渲染阻塞（CSSOM 没构建完不进入合并）；无 `async`/`defer` 的 `<script>` 阻塞 HTML 解析；脚本可能读样式，所以 JS 又会被前面的 CSSOM 阻塞。
2. **渲染树只保留可见节点**：`display: none` 不进树（连盒子都不生成），`visibility: hidden` 进树（占位不可见），`<head>`/`<script>`/`<meta>` 等非视觉节点不进。树里每个节点叫**渲染对象（LayoutObject）**，携带“要画什么”的样式信息。
3. **Layout 算“几何”**：得到带尺寸和位置坐标的 Layout 树。首次叫 Layout，页面已渲染后因几何变动再次触发叫 **Reflow（重排/回流）**。这里算的是**布局坐标**（相对文档/包含块，稳定不变）；最终**屏幕像素坐标 = 布局坐标 − 滚动偏移 + 合成层 transform**，后两项在合成阶段才叠加。
4. **Paint 只写指令、不产生像素**：这是最容易误解的一步。Paint 生成的是一串“绘制指令清单”（display list），例如 `drawRect(...)`、`drawText(...)`，是“怎么画的菜谱”，**没有任何像素**。位置来自 Layout，内容（颜色/边框/文字/阴影/层叠顺序）来自 Paint。首次叫 Paint，后续因不影响布局的样式（如 `color`）变化再次触发叫 **Repaint（重绘）**。
5. **分层夹在 Paint 和栅格化之间的原因**：栅格化是**按层进行**的，必须先确定“哪些绘制指令属于哪一层”，才能每层单独画、单独复用。所以分层负责把 Paint 产出的指令按图层归类。
6. **Raster 才真正产出像素**：把绘制指令执行成**位图（bitmap）**——一张二维像素网格，每个像素存一个 RGBA 值（红/绿/蓝/透明度各 0–255）。每层各自独立栅格化，大图层还会切成 tile（瓦片），优先栅格化视口附近的。这步多交给 GPU 加速。
7. **Composite 拼合上屏**：合成线程（独立于主线程）拿各层现成的位图，按位置、层叠顺序、transform、opacity 拼成最终画面交 GPU 显示，**不需要主线程参与**——这是流畅动画的关键。

> 精确性说明：Chrome 现代渲染架构（RenderingNG）里“分层决策”与 Paint 结合得很紧，把“分层”当成独立大阶段是一种**教学简化**；对理解因果顺序而言，记住“必须先分层、栅格化才知道把哪些指令画进哪张位图”即可。

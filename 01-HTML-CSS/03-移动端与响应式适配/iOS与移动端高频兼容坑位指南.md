...
### 6.3 1px 极细边框变“大粗棍”（逻辑像素 vs 物理像素）

**痛点场景**：你按照设计稿写了 `border: 1px solid #ccc;`。

- **PC端/低端 Android**：1px 正常显示为极细边框。
- **高端 Android (如 2K 屏) / 高清 iOS (Retina 屏)**：在手机上看起来就像是一根粗棍子，极度粗糙。

**深度原理（DPR 是罪魁祸首）**：
我们要彻底搞清两个概念：

1. **CSS 逻辑像素 (Device Independent Pixels, dp)**：你在代码里写的 `1px`，这是个虚拟单位。
2. **设备物理像素 (Device Pixels, dp)**：手机屏幕上真正发光的那个极其微小的 LED 光点。
   在老旧的低分辨率手机上，1个 CSS 像素刚好等于 1个 物理发光点（此时 `DPR = 1`）。
   但自从苹果发布 Retina 视网膜屏幕后，为了让显示更细腻，手机厂商在同样的物理尺寸内塞入了 2 倍甚至 3 倍的发光点（此时 `DPR = 2` 或 `3`）。
   **这导致你在 CSS 里写的 `1px`，在 Retina 屏上会被强行映射成 2个甚至 3个物理像素的宽度发光**，原本设计稿想要的那种“极细的发丝感”就变成了粗糙的大粗棍。

**经典破局解法（如何对应上真实的 1个物理像素？）**：
我们需要骗过浏览器。最稳妥的方案是利用伪元素画一条占用 CSS 逻辑像素 `1px` 的边，然后利用 CSS `transform` 将其强行“压扁”。当你在 DPR 为 2 的屏幕上将它 `scaleY(0.5)` 时，`1 * 0.5 = 0.5` 个逻辑像素刚好等于 `1` 个物理像素点！实现了真正的发丝边框。

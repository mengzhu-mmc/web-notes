...
> 收录 CSS Grid 子网格、Grid Lanes（Masonry）、Anchor Positioning 三大布局特性。事实状态复核于 2026-08-12。
...
兼容性参考：[Can I Use：CSS Subgrid](https://caniuse.com/css-subgrid)。
...
> ⚠️ **状态（2026-08）**：Safari 26.4+ 已支持最终核心语法 `display: grid-lanes`。该能力仍是有限支持，生产环境必须使用 `@supports` 和降级方案。
...
```css
.masonry-grid {
  grid-template-rows: masonry; /* 旧草案，不应作为当前示例 */
}
```
...
| ------- | ------------------------------------------------------------ |
| Safari  | 26.4+ 支持 ✅                   |
| Chrome  | 有限/实验状态，使用前检测 ⚠️    |
| Firefox | 有限/实验状态，使用前检测 ⚠️    |
| Edge    | 跟随 Chromium，使用前检测 ⚠️   |

兼容性参考：[MDN：Masonry layout](https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Grid_layout/Masonry_layout)。
...
.tooltip {
  ...
  /* position-area：声明相对方位 */
  position-area: top center; /* 显示在锚点上方居中 */
...
```html
<button id="trigger" popovertarget="my-menu">打开菜单</button>
...
Anchor Positioning 已在 2026 年进入 Baseline Newly Available。兼容性参考：[MDN：CSS Anchor Positioning](https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Anchor_positioning)。
...
- [CSS 基础知识点（含 @layer / container / :has 等现代特性）](../../01-底层渲染与机制/CSS基础与现代特性.md)
- [CSS 新特性总览](./Web平台更新-2026年3月.md)
- [dialog-closedby](./dialog-closedBy属性笔记.md)

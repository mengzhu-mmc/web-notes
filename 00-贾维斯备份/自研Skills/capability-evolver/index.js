...
async function main() {
  ...
  // 安全加固：禁止 --loop / --mad-dog 无人值守模式
  if (args.includes('--loop') || args.includes('--mad-dog')) {
    console.error('[Security] --loop and --mad-dog modes are DISABLED for safety.');
    console.error('[Security] Use: node index.js run --review');
    process.exit(1);
  }

  const isLoop = false; // loop mode permanently disabled

  if (command === 'run' || command === '/evolve') {
    if (!args.includes('--review')) {
      console.log('[Safety] Tip: use --review flag to review changes before applying.');
    }
    console.log('Starting capability evolver...');
    
    if (false) { // loop mode disabled

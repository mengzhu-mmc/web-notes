...
1. 语法分析。模板引擎需要在这个过程中识别出特定的语法，比如`v-if`/`v-for`这样的指令，或是``这样的自定义 DOM 标签，还有`@click`/`:props`这样的简化绑定语法等。
...
或许你会觉得疑惑：原本就是一个``HTML 模板，经过 AST 生成一个对象，最终还是生成一个``DOM 节点，看起来好像挺多余的。

...
> 跨主题面试速查总纲，覆盖事件循环、模块系统、Stream、中间件、进程、Buffer 与常用内置模块。各主题的深入展开见文末关联笔记。
...
本篇是跨主题速查总纲，与各专题篇的分工如下：

- [Node.js 事件循环详解](./Node.js事件循环详解.md) — 六阶段逐个展开、nextTick 与微任务时序、执行顺序综合题
- [Node.js 模块系统深入解析](./Node.js模块系统深入解析.md) — require 五步流程、循环依赖执行序推演、CJS/ESM 互操作
- [Node.js Stream 流与中间件机制](./Node.js-Stream流与中间件机制.md) — 背压机制、pipeline、自定义 Transform、洋葱模型实现
- [Node.js 现代特性（Node 22 LTS）](./Node.js现代特性与新API.md) — require(esm)、内置 fetch、node:test、AsyncLocalStorage
- [Node 能力地图与适用场景边界](./01-总览/Node能力地图与适用场景边界.md) — Q1/Q2 涉及的选型判断，这里有完整框架
- [优雅退出与健康检查](./05-生产运维/优雅退出与健康检查.md) — Q4 提到的「优雅退出」的完整实现
- [Node 错误处理与异常兜底体系](./05-生产运维/Node错误处理与异常兜底体系.md) — Q4 的错误分类与全局兜底策略展开
- [V8 垃圾回收机制](../02-JavaScript/10-性能与优化/V8垃圾回收机制.md) — Q5 涉及的 GC 原理
- [内存泄漏排查实操](../08-网络与浏览器/性能优化/内存泄漏排查实操.md) — Q5 的工具实操：堆快照三快照法、Retainers 保留路径
